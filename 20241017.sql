...
CREATE TABLE CAR(
	...
	CAR_MAKER VARCHAR2(200) NOT NULL,
	CAR_MAKE_YEAR NUMBER(4) NOT NULL,
	CAR_PRICE NUMBER(5) NOT NULL
);

ALTER TABLE CAR ADD CONSTRAINT CAR_ID_PK PRIMARY KEY (CAR_ID);


SELECT * FROM BOARD_COMMENT_VIEW ORDER BY CDATE DESC;

SELECT * FROM BOARD_COMMENT_VIEW WHERE BNO = 999 ORDER BY cdate DESC;

SELECT ROW_NUMBER() OVER(ORDER BY CDATE DESC), BCV. * FROM BOARD_COMMENT_VIEW BCV WHERE BNO = 999;


SELECT * FROM
	(SELECT ROW_NUMBER() OVER(ORDER BY CDATE DESC) AS RW, BCV.*
	FROM BOARD_COMMENT_VIEW BCV WHERE BNO = 999)
